...
- `scripts/text_engine_pipeline.py`: OCR orchestration, masks, LaMa routing, deterministic rendering, page isolation, reports, and exact-name output mapping.
...
LaMa model weights are a runtime cache, not a second project. Resolve them in this order: `REPAIR_COMIC_LAMA_ROOT`, `runtime/lama` inside the Skill, then the existing `D:\AI\LaMa_IOPaint` cache. If IOPaint or its model is unavailable for a complex-background block, mark the page `evidence_blocked`; never fall back to OpenCV inpaint or a flat patch on artwork.
...
5. Remove the complete ordinary-text block. Use deterministic flat/background fill only where it cannot alter artwork. Use LaMa for texture or artwork overlap. Reject residual glyphs, seams, broken line art, or background damage.
...
- Never use a rectangular white patch, weak OpenCV inpaint, or full-page image generation merely to repair ordinary text.
